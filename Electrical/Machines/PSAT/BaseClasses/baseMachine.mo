...
partial model baseMachine "Base machine for PSAT models"
  ...
  Modelica.Blocks.Interfaces.RealOutput delta(
    ...
    quantity="Angle",
    unit="rad",
    displayUnit="rad",fixed=true) "Rotor angle" annotation (Placement(transformation(
        origin={110,60},
        extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Interfaces.RealOutput w(start=1,fixed=true) "Rotor speed [pu]"
